...
//!
//! Requests go through [`crate::atlassian_http`], which is also what the Jira
//! issue calls in `src/jira.rs` use — one definition of how the token reaches
//! curl (on **stdin** via `-K -`, never argv) and one place every call is
//! recorded for the Logs tab trace.
...
// The trace lives with the request that fills it (`src/atlassian_http.rs`),
// now that Jira issue calls land in it too. Re-exported here because the Logs
...
/// handling and the Logs-tab trace live there, shared with the Jira issue
/// calls in `src/jira.rs`, so there is exactly one definition of how a token
/// reaches curl.
fn curl_request(
    auth: &AlertsAuth,
    url: &str,
