...
const API_TRACE_LEN: usize = 5;

/// A recorded alerts API call, for the **Jira Alerts** trace in the Logs tab.
///
...
pub struct ApiCall {
    ...
    /// ever resident — the feed is up to 50 alerts a page and the app does
    /// not control how large that is, so keeping five would be unbounded in
    /// practice.
...
mod tests {
    ...
    /// `curl_request`, which needs the network).
    #[test]
