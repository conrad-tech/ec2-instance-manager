...
# Every directory on the shared mount, by owner. A home can outlive its
# account -- deleted on both boxes, the directory stays, owned by a number no
# passwd entry mentions. Handing that number to somebody new would give them
# the files, so the allocator floors above these too.
for d in /efs/home/*/; do [ -d "$d" ] && stat -c "HOMEOWN %u %g" "$d"; done 2>/dev/null
